  ...
  return (
    ...
    >
      <div className="relative  w-full h-full  aspect-video flex flex-row items-center justify-center">
        <img className="w-full h-full  object-center object-fill absolute drop-shadow-amber-400 drop-shadow-2xl " src={img} alt={title} />
      </div>
    </figure>
  );
};
...
function SectionPartner() {
  return (
    ...
    <div className="relative flex w-full flex-col items-center backdrop-brightness-100 backdrop-blur-xs backdrop-opacity-95  justify-center py-10 px-4  overflow-hidden">
      <TitleDescript title="Nos Partenaires et Références" descript="Des collaborations solides pour des solutions innovantes" className="text-center mb-10"/>
      ...
      <div className="pointer-events-none absolute inset-y-0 left-0 w-1/4 bg-gradient-to-r from-it4a-secondary"/>
      <div className="pointer-events-none absolute inset-y-0 right-0 w-1/4 bg-gradient-to-l from-it4a-orange/50"/>
    </div>
  );
}
